...
#'
lp_nl_prueba <- function(endog_data,
                         lags_endog_lin = NULL,
                         lags_endog_nl  = NULL,
                         lags_criterion = NaN,
                         max_lags       = NaN,
                         ...
                         switching_var1 = NULL,
                         switching_var2 = NULL,
                         lag_switching_var1  = TRUE,
                         lag_switching_var2 = TRUE,
                         use_logistic_var1   = TRUE,
                         use_logistic_var2 = TRUE,
                         use_hp_var1    = NULL,
                         use_hp_var2    = NULL,
                         lambda_var1    = NULL,
                         lambda_var2    = NULL,
  ...
  specs$dep_var_pos    <- dep_var_pos
  specs$trend          <- trend
  specs$shock_type     <- shock_type
  specs$confint        <- confint
  specs$hor            <- hor
  specs$switching_var1 <- switching_var1
  specs$switching_var2 <- switching_var2
  specs$lag_switching_var1  <- lag_switching_var1
  specs$lag_switching_var2  <- lag_switching_var2
  specs$use_logistic_var1   <- use_logistic_var1
  specs$use_logistic_var2  <- use_logistic_var2
  specs$use_hp_var1    <- use_hp_var1
  specs$use_hp_var2    <- use_hp_var2
  specs$lambda_var1    <- lambda_var1
  specs$lambda_var2    <- lambda_var2
  specs$gamma_var1     <- gamma_var1
  specs$gamma_var2     <- gamma_var2
  ...
  # Check whether switching variable 1 is given
  if(is.null(specs$switching_var1)){
    stop('Please provide the first switching variable.')
  }
  
  # Check wether switching variable 2 is given
  if(is.null(specs$switching_var2)) {
    stop('Please provide the second switching variable.')
  }
  ...
  if(isTRUE(specs$use_hp_var1) & (is.null(specs$lambda_var1))){
    stop('Please specify lambda for the HP-filter of var 1.')
  }
  
  # Check whether lambda is given if 'use_hp == 1'
  if(isTRUE(specs$use_hp_var2) & (is.null(specs$lambda_var2))){
    stop('Please specify lambda for the HP-filter of var 2.')
  }
  ...
  if(isTRUE(use_logistic_var1) & is.null(gamma_var1) == TRUE){
    stop('Please specify gamma for the transition function of first variable.')
  }
  
  if(isTRUE(use_logistic_var2) & is.null(gamma_var2) == TRUE){
    stop('Please specify gamma for the transition function of second variable.')
  }
  
  # Check whether gamma is positive
  if(isTRUE(use_logistic_var1)){
    if(specs$gamma_var1 < 0){
      stop('Gamma of first variable has to be a positive number.')
    }
  }
  
  # Check whether gamma is positive
  if(isTRUE(use_logistic_var2)){
    if(specs$gamma_var2 < 0){
      stop('Gamma of second variable has to be a positive number.')
    }
  }
  
  # Check whether use_hp is either 0 or 1 is positive
  if(isTRUE(use_logistic_var1)){
    if(!(specs$use_hp_var1 %in% c(0, 1))){
      stop('Please set use_hp_var1 = 0 (do not use HP-filter), or use_hp_var1 = 1 (use HP-filter) in first variable.')
    }
  }
  
  if(isTRUE(use_logistic_var2)){
    if(!(specs$use_hp_var2 %in% c(0, 1))){
      stop('Please set use_hp_var2 = 0 (do not use HP-filter), or use_hp_var2 = 1 (use HP-filter) in second variable.')
    }
  }
  ...
  data_nl     <- create_nl_data_prueba(specs, endog_data)  %>%
    stats::na.omit()                   %>%
    `rownames<-`(NULL)
  ...
  data_lin    <- create_lin_data(specs, endog_data)                                ###  Checar que create_lin_data bien implementado####
  y_lin       <- data_lin[[1]]
  x_lin       <- data_lin[[2]]
  dep_var_pos <- specs$dep_var_pos
  ...
  irf_temp_s1_mean_var1  <-  matrix(NaN, specs$endog, specs$hor + 1)
  irf_temp_s1_low_var1   <-  irf_temp_s1_mean_var1
  irf_temp_s1_up_var1    <-  irf_temp_s1_mean_var1
  
  irf_temp_s2_mean_var1  <-  matrix(NaN, specs$endog, specs$hor + 1)
  irf_temp_s2_low_var1   <-  irf_temp_s2_mean_var1
  irf_temp_s2_up_var1    <-  irf_temp_s2_mean_var1
  
  irf_temp_s1_mean_var2  <-  matrix(NaN, specs$endog, specs$hor + 1)
  irf_temp_s1_low_var2   <-  irf_temp_s1_mean_var2
  irf_temp_s1_up_var2    <-  irf_temp_s1_mean_var2
  
  irf_temp_s2_mean_var2  <-  matrix(NaN, specs$endog, specs$hor + 1)
  irf_temp_s2_low_var2   <-  irf_temp_s2_mean_var2
  irf_temp_s2_up_var2    <-  irf_temp_s2_mean_var2
  
  # Arrays to store irfs
  irf_s1_mean_var1  <-  array(NaN, dim = c(specs$endog, specs$hor + 1, specs$endog))
  irf_s1_low_var1   <-  irf_s1_mean_var1
  irf_s1_up_var1    <-  irf_s1_mean_var1
  
  irf_s2_mean_var1  <-  array(NaN, dim = c(specs$endog, specs$hor + 1, specs$endog))
  irf_s2_low_var1   <-  irf_s2_mean_var1
  irf_s2_up_var1    <-  irf_s2_mean_var1
  
  irf_s1_mean_var2  <-  array(NaN, dim = c(specs$endog, specs$hor + 1, specs$endog))
  irf_s1_low_var2   <-  irf_s1_mean_var2
  irf_s1_up_var2    <-  irf_s1_mean_var2
  
  irf_s2_mean_var2  <-  array(NaN, dim = c(specs$endog, specs$hor + 1, specs$endog))
  irf_s2_low_var2   <-  irf_s2_mean_var2
  irf_s2_up_var2    <-  irf_s2_mean_var2
  
  # Matrices to store OLS parameters for regime 1 & 2 & 3 & 4        
  b1_s1_var1      <- matrix(NaN, specs$endog, specs$endog)
  b1_low_s1_var1  <- matrix(NaN, specs$endog, specs$endog)
  b1_up_s1_var1   <- matrix(NaN, specs$endog, specs$endog)
  
  b1_s2_var1      <- matrix(NaN, specs$endog, specs$endog)
  b1_low_s2_var1  <- matrix(NaN, specs$endog, specs$endog)
  b1_up_s2_var1   <- matrix(NaN, specs$endog, specs$endog)
  
  b1_s1_var2      <- matrix(NaN, specs$endog, specs$endog)
  b1_low_s1_var2  <- matrix(NaN, specs$endog, specs$endog)
  b1_up_s1_var2   <- matrix(NaN, specs$endog, specs$endog)
  
  b1_s2_var2      <- matrix(NaN, specs$endog, specs$endog)
  b1_low_s2_var2  <- matrix(NaN, specs$endog, specs$endog)
  b1_up_s2_var2   <- matrix(NaN, specs$endog, specs$endog)
  ...
  start_nl_s3 <- 2 + 2 * specs$endog * specs$lags_endog_nl  # Adjusted for two sets of regimes before it
  end_nl_s3 <- start_nl_s3 + specs$endog - 1
  samp_nl_s3 <- start_nl_s3:end_nl_s3
  
  start_nl_s4 <- 2 + 3 * specs$endog * specs$lags_endog_nl  # Adjusted for three sets of regimes before it
  end_nl_s4 <- start_nl_s4 + specs$endog - 1
  samp_nl_s4 <- start_nl_s4:end_nl_s4
  ...
  if(is.nan(specs$lags_criterion) == TRUE) {
    ...
    nl_irfs <- foreach( s        = 1:specs$endog, .packages = c("lpirfs", "dplyr", "rlang")) %dopar%{ # Accounts for the shocks
      
      for (h in 1:specs$hor){   # Accounts for the horizons
        
        if(is_true(specs$cumul_mult)) {
          
          
          yy    <- dplyr::lead(y_nl, (h )) - dplyr::lag(y_nl, 1)
          yy_xx <- cbind(yy, x_nl) %>%
            stats::na.omit()
        ...
        for (k in 1:specs$endog){ # Accounts for the reactions of the endogenous variables
          ...
          b1_s1_var1[k, ]       <-   b[samp_nl_s1]
          b1_low_s1_var1[k, ]   <-   b[samp_nl_s1] - std_err[samp_nl_s1]
          b1_up_s1_var1[k, ]    <-   b[samp_nl_s1] + std_err[samp_nl_s1]
          
          b1_s2_var1[k, ]       <-   b[samp_nl_s2]
          b1_low_s2_var1[k, ]   <-   b[samp_nl_s2] - std_err[samp_nl_s2]
          b1_up_s2_var1[k, ]    <-   b[samp_nl_s2] + std_err[samp_nl_s2]
          
          b1_s1_var2[k, ]       <-   b[samp_nl_s3]
          b1_low_s1_var2[k, ]   <-   b[samp_nl_s3] - std_err[samp_nl_s3]
          b1_up_s1_var2[k, ]    <-   b[samp_nl_s3] + std_err[samp_nl_s3]
          
          b1_s2_var2[k, ]       <-   b[samp_nl_s4]
          b1_low_s2_var2[k, ]   <-   b[samp_nl_s4] - std_err[samp_nl_s4]
          b1_up_s2_var2[k, ]    <-   b[samp_nl_s4] + std_err[samp_nl_s4]
          ...
          diagnost_each_k[k, 4]     <- stats::pf(get_diagnost[[5]], get_diagnost[[6]], get_diagnost[[7]], lower.tail = F)
          
        }
        
        # Estimate local projections
        irf_temp_s1_mean_var1[, h + 1] <- t(b1_s1_var1        %*%  d[ , s])
        irf_temp_s1_low_var1[,  h + 1] <- t(b1_low_s1_var1    %*%  d[ , s])
        irf_temp_s1_up_var1[,   h + 1] <- t(b1_up_s1_var1     %*%  d[ , s])
        
        irf_temp_s2_mean_var1[, h + 1] <- t(b1_s2_var1        %*%  d[ , s])
        irf_temp_s2_low_var1[,  h + 1] <- t(b1_low_s2_var1    %*%  d[ , s])
        irf_temp_s2_up_var1[,   h + 1] <- t(b1_up_s2_var1     %*%  d[ , s])
        
        irf_temp_s1_mean_var2[, h + 1] <- t(b1_s1_var2        %*%  d[ , s])
        irf_temp_s1_low_var2[,  h + 1] <- t(b1_low_s1_var2    %*%  d[ , s])
        irf_temp_s1_up_var2[,   h + 1] <- t(b1_up_s1_var2     %*%  d[ , s])
        
        irf_temp_s2_mean_var2[, h + 1] <- t(b1_s2_var2        %*%  d[ , s])
        irf_temp_s2_low_var2[,  h + 1] <- t(b1_low_s2_var2    %*%  d[ , s])
        irf_temp_s2_up_var2[,   h + 1] <- t(b1_up_s2_var2     %*%  d[ , s])
      ...
      list(irf_temp_s1_mean_var1, irf_temp_s1_low_var1, irf_temp_s1_up_var1,
           irf_temp_s2_mean_var1, irf_temp_s2_low_var1, irf_temp_s2_up_var1,
           irf_temp_s1_mean_var2, irf_temp_s1_low_var2, irf_temp_s1_up_var2,
           irf_temp_s2_mean_var2, irf_temp_s2_low_var2, irf_temp_s2_up_var2,
           diagnost_ols_each_h)
    ...
    for(i in 1:specs$endog){
      ...
      # Fill irfs
      irf_s1_mean_var1[, , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][1]))
      irf_s1_low_var1[, ,  i] <- as.matrix(do.call(rbind, nl_irfs[[i]][2]))
      irf_s1_up_var1[, ,   i] <- as.matrix(do.call(rbind, nl_irfs[[i]][3]))
      
      irf_s2_mean_var1[, , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][4]))
      irf_s2_low_var1[,  , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][5]))
      irf_s2_up_var1[,   , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][6]))
      
      irf_s1_mean_var2[, , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][7]))
      irf_s1_low_var2[, ,  i] <- as.matrix(do.call(rbind, nl_irfs[[i]][8]))
      irf_s1_up_var2[, ,   i] <- as.matrix(do.call(rbind, nl_irfs[[i]][9]))
      
      irf_s2_mean_var2[, , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][10]))
      irf_s2_low_var2[,  , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][11]))
      irf_s2_up_var2[,   , i] <- as.matrix(do.call(rbind, nl_irfs[[i]][12]))
      
      # First value of is merely the shock                  ## Aqui chance puede haber error
      irf_s1_mean_var1[, 1, i]   <- t(d[, i])
      irf_s1_low_var1[,  1, i]   <- irf_s1_mean_var1[, 1, i]
      irf_s1_up_var1[,   1, i]   <- irf_s1_mean_var1[, 1, i]
      
      irf_s2_mean_var1[, 1, i]   <- t(d[, i])
      irf_s2_low_var1[,  1, i]   <- irf_s1_mean_var1[, 1, i]
      irf_s2_up_var1[,   1, i]   <- irf_s1_mean_var1[, 1, i]
      
      irf_s1_mean_var2[, 1, i]   <- t(d[, i])
      irf_s1_low_var2[,  1, i]   <- irf_s1_mean_var2[, 1, i]
      irf_s1_up_var2[,   1, i]   <- irf_s1_mean_var2[, 1, i]
      
      irf_s2_mean_var2[, 1, i]   <- t(d[, i])
      irf_s2_low_var2[,  1, i]   <- irf_s1_mean_var2[, 1, i]
      irf_s2_up_var2[,   1, i]   <- irf_s1_mean_var2[, 1, i]
      
      # Fill list with all OLS diagnostics
      diagnostic_list[[i]]        <- nl_irfs[[i]][[13]]
    ...
    nl_irfs <- foreach(s         = 1:specs$endog,
                       .packages = c("lpirfs", "dplyr")) %dopar% { # Accounts for shocks
    ...
    for(i in 1:specs$endog){
      ...
      # Fill list with all OLS diagnostics
      diagnostic_list[[i]]        <- nl_irfs[[i]][[13]]
      chosen_lags_list[[i]]       <- nl_irfs[[i]][[14]]
  ...
  result         <-  list(irf_s1_mean_var1     = irf_s1_mean_var1,
                          irf_s1_low_var1      = irf_s1_low_var1,
                          irf_s1_up_var1       = irf_s1_up_var1,
                          irf_s2_mean_var1     = irf_s2_mean_var1,
                          irf_s2_low_var1      = irf_s2_low_var1,
                          irf_s2_up_var1       = irf_s2_up_var1,
                          irf_s1_mean_var2     = irf_s1_mean_var2,
                          irf_s1_low_var2      = irf_s1_low_var2,
                          irf_s1_up_var2       = irf_s1_up_var2,
                          irf_s2_mean_var2     = irf_s2_mean_var2,
                          irf_s2_low_var2      = irf_s2_low_var2,
                          irf_s2_up_var2       = irf_s2_up_var2,
                          fz_var1              = fz_var1,
                          fz_var2              = fz_var2,
                          specs           = specs,
